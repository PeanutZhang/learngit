...
//        channel.setVibrationPattern(new long[]{1000, 2000, 1000,3000});
    ...
    public static void show(Context context, int largeIcon,
        ...
        //设置自动收报机和通知中显示的大图标。
        builder.setLargeIcon(BitmapFactory.decodeResource(context.getResources(), largeIcon == 0 ? R.drawable.ic_stat_name : largeIcon))
                .setSmallIcon(smallIcon == 0 ? R.drawable.ic_stat_name : smallIcon)
                .setContentText(TextUtils.isEmpty(contentText) ? null : contentText)
                .setContentTitle(TextUtils.isEmpty(contentTitle) ? null : contentTitle)
                // APP名称的副标题
                .setSubText(TextUtils.isEmpty(subText) ? null : subText)
                //设置优先级 PRIORITY_DEFAULT
                .setPriority(priority)
                // 设置通知首次弹出时，状态栏上显示的文本
                .setTicker(TextUtils.isEmpty(ticker) ? Ticker : ticker)
                .setContent(view)
                .setColor(Color.BLUE)
                // 设置通知发送的时间戳
